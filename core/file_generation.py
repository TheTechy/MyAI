...
def extract_and_generate(llm_output: str) -> list[dict]:
    ...
    for match in FILE_BLOCK_RE.finditer(llm_output):
        ...
        disk_path = _save_to_disk(filename, file_bytes)
...
7. ONLY emit [FILE:] tags when the user has explicitly asked you to create, save, write, or generate a file. Reviews, analysis, explanations, suggestions, rewrites, and code feedback are ALWAYS inline responses — never files. When in doubt, respond inline.
...
10. If the user has already specified both the file type AND the content in their request, generate the file IMMEDIATELY. Do NOT ask clarifying questions such as "Would you like me to save this as a file?" or "What format would you like?" — the user has already told you. Generate it now.
