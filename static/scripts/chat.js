...
const allowedInput	= [".txt",".md",".py",".js",".ts",".jsx",".tsx",".cs",".cpp",".c",".java",".json",".yaml",".yml",".sh",".sql",".xml",".csv",".toml",".html",".css",".pdf",".docx",".xlsx", ".jpg",".jpeg",".png",".webp",".gif"]
...
const COLLAPSE_KEY = 'myai_sidebar_collapsed';
...
// Clicking the collapsed search icon expands the sidebar and focuses the input
convSearchEl.closest('.sidebar-search').addEventListener('click', () => {
...
function renderInline(text) {
  return text.split(/(`[^`\n]+`)/g).map((part, i) => {
    ...
    // Extract links before escaping so URLs aren't mangled
    const linked = part.replace(
      /\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g,
      (_, label, url) => `\x00LINK\x00${escHtml(label)}\x00${url}\x00`
    );
    return escHtml(linked)
 ...
 */
function appendAiBubbleStatic(text, files = []) {
  hideEmpty();
  const row = document.createElement('div');
  row.className = 'msg-row ai';
  ...
  // Re-attach any download cards stored against this message
  if (files && files.length) {
    const bubble = row.querySelector('.bubble');
    for (const f of files) {
      renderFileCard(f, bubble);
    }
  }
}
...
function renderConvList(convs, query = '') {
  convListEl.innerHTML = '';

  if (convs.length === 0) {
    if (query) {
      const empty = document.createElement('div');
      empty.className   = 'conv-search-empty';
  ...
  // When searching, skip date grouping — just show a flat list
  if (query) {
    convs.forEach(c => convListEl.appendChild(makeConvItem(c, query)));
    return;
  }
...
async function performSearch(query) {
  try {
    const url  = query
      ? `/conversations/${encodeURIComponent(userName)}/search?q=${encodeURIComponent(query)}`
      : `/conversations/${encodeURIComponent(userName)}`;
    const res  = await fetch(url);
    const convs = await res.json();
    renderConvList(convs, query);
  } catch (e) {
...
if (convSearchEl) {
  convSearchEl.addEventListener('input', () => {
    ...
    convSearchClear.style.display = q ? 'flex' : 'none';
...
function makeConvItem({ conv_id, title }, query = '') {
  ...
  if (query) {
    titleSpan.appendChild(highlightMatch(title, query));
  } else {
  ...
  delBtn.title     = 'Delete';
  delBtn.innerHTML = `<svg viewBox="0 0 12 12" xmlns="http://www.w3.org/2000/svg"><path d="M1 1l10 10M11 1L1 11"/></svg>`;
  ...
  try {
    ...
    if (messages.length === 0) {
      ...
      for (const msg of messages) {
        ...
        else                     appendAiBubbleStatic(msg.content, msg.files || []);
      }
      scrollToBottom();
    }
  } catch (e) {
...
// ── File download card ────────────────────────────────────────────────────────
// Accepts two shapes:
//   Live (from SSE):    { filename, mime_type, size, data }       // base64 blob
//   Restored (history): { filename, mime_type, size, file_id }    // server URL
function renderFileCard(file, bubble) {
  const { filename, mime_type, size, data, file_id } = file;

  // Build the download URL — either a blob (live) or a server route (restored)
  let url;
  if (data) {
    ...
    url = `/files/${encodeURIComponent(file_id)}/download`;
  } else {
  ...
  // Attach after the bubble's row, or append to chat if no bubble exists
  if (bubble) {
...
(function wireAttachButton() {
  ...
  fileInput.type   = "file";
  fileInput.accept = allowedInput.join(",");
...
async function sendPrompt(promptText) {
  ...
  // A single event may have multiple data: lines (e.g. large base64 payloads)
  // that must be concatenated before parsing.
  function processEvents(buf) {
    const events = buf.split('\n\n');
    // Last element may be incomplete — return it to be buffered
    ...
    for (const block of events) {
      ...
      if (eventType && dataLines.length) {
        ...
        dispatchSSEEvent(eventType, payload);
      }
    }
    return remaining;
  }

  function dispatchSSEEvent(eventType, payload) {
        ...
        if (eventType === 'status') {
          const textEl = _thinkBubble?.querySelector('.thinking-text');
...
// ── Voice input (faster-whisper via /transcribe) ─────────────────────────────
(function wireVoiceInput() {
  ...
  // ── Tuning knobs ───────────────────────────────────────────────────────────
  // MAX_RECORDING_MS  — hard cut-off; recording auto-stops after this long.
  //                     Raise if you give long dictations, lower to force brevity.
  // CHUNK_INTERVAL_MS — how often MediaRecorder flushes audio data internally.
  //                     Lower = less tail latency after stop(); 100 ms is fine.
  const MAX_RECORDING_MS  = 120_000;   // 2 min safety cut-off
  const CHUNK_INTERVAL_MS = 100;       // flush chunks every 100 ms
  ...
  // ── Mic status pill (appears below the input bar while active) ────────────
  let statusPill = null;

  function showMicPill(msg) {
    if (!statusPill) {
      statusPill = document.createElement('div');
      statusPill.id = 'mic-status-pill';
      // Insert just above the input row; fall back to appending to chat
      const inputRow = inputEl.closest('form, .input-row, .input-area, .input-wrap')
                    ?? inputEl.parentElement;
      inputRow.insertAdjacentElement('beforebegin', statusPill);
    }
  ...
  function setMicState(state) {
    // state: 'idle' | 'recording' | 'uploading' | 'transcribing'
    ...
    micBtn.title = labels[state] ?? state;
  }

  // ── Stop recording and ship to /transcribe ────────────────────────────────
  async function stopAndTranscribe() {
    if (!mediaRecorder || mediaRecorder.state === 'inactive') return;

    clearTimeout(recordingTimer);
    setMicState('uploading');
    showMicPill('⏳ Sending audio…');

    // stop() is async — onstop fires once the final chunk is flushed
    ...
    showMicPill('🎙 Transcribing… (first run loads the model)');
    ...
    try {
      ...
      if (!res.ok || data.error) {
        ...
        inputEl.value = before + spacer + data.text + after;
        inputEl.dispatchEvent(new Event('input'));
        inputEl.focus();
        console.debug(`[voice] transcribed in ${elapsed}s`);
      } else {
        showMicPill('No speech detected');
        setTimeout(hideMicPill, 2000);
      }
    } catch (err) {
  ...
  async function startRecording() {
    ...
    showMicPill('🔴 Recording… click mic to stop');

    recordingTimer = setTimeout(() => {
      if (mediaRecorder && mediaRecorder.state !== 'inactive') stopAndTranscribe();
    }, MAX_RECORDING_MS);
  }
